const { tb_seminar_proposal } = require("../../models");
...
class ProposalController {
  async InsertData(req, res,next) {
    ...
    console.log(item)
    const dtSAnggota = await tb_seminar_proposal.findOne({
    ...
    if (dtSAnggota) {
      ...
      dtAnggota = await tb_seminar_proposal.create(item)
      .then(res=>id=res.id)
  ...
  async UpdateData(req, res) {
    ...
    if (req.params.id == null) {
      ...
      if (!dtSAnggota) {
        ...
        dtAnggota = await tb_seminar_proposal.update(update, {
          where: { nim: req.params.id }
        });
        status = 200;
        message = "Sukses";
  ...
  async DeleteData(req, res) {
    ...
    if (req.params.id == null) {
      ...
      dtAnggota = await tb_seminar_proposal.destroy({ where: { id: req.params.id } });
    }
    if (!dtAnggota) {
      status = 404;
      message = "Data Member Tidak Ditemukan";
