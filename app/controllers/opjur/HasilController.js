...
class HasilController {
  async InsertData(req, res, next) {
    ...
    console.log(item)
    const dtSAnggota = await tb_seminar_hasil.findOne({
  ...
  async UpdateData(req, res) {
    ...
    if (req.params.id == null) {
      ...
      if (!dtSAnggota) {
        ...
        dtAnggota = await tb_seminar_hasil.update(update, {
          where: { id: req.params.id }
        });
        status = 200;
        message = "Sukses";
  ...
  async DeleteData(req, res) {
    ...
    if (req.params.id == null) {
      ...
      dtAnggota = await tb_seminar_hasil.destroy({ where: { id: req.params.id } });
    }
    if (!dtAnggota) {
      status = 404;
      message = "Data Member Tidak Ditemukan";
...
const hasilController = new HasilController();
