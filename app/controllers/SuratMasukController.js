const { tb_surat_masuk } = require("../models");
class SuratMasukController {
  async  Pengajuan(req, res) {
    ...
    console.log(item)
      ...
      dtAnggota = await tb_surat_masuk.create(item);
      status = 200;
  ...
  async SelectData(req, res) {
    ...
    if (req.params.id == null) {
      dtAnggota = await tb_surat_masuk.findAll({ order: [["id", "ASC"]] });
    } else {
      dtAnggota = await tb_surat_masuk.findOne({
  ...
  async disposisi(req, res) {
    ...
    if (req.params.id == null) {
      ...
      const dtSAnggota = await tb_surat_masuk.findOne({
        where: { id: req.params.id }
      });
      ...
      if (!dtSAnggota) {
        ...
        dtAnggota = await tb_surat_masuk.update(update, {
          where: { id: req.params.id }
        });
        status = 200;
        message = "Sukses";
